<?php
    function changeLanguageDropDown()
    {
        ...
        echo '<label for="language">Bahasa:</label>';

        echo '<select name="language" id="language" onchange="changeLanguage()">';
        echo '<option value="indonesian"' . (strpos($url, "indonesian")!==false ? ' selected ' : '' ) . '>Bahasa Indonesia</option>';
        echo '<option value="english"'. (strpos($url, "english")!==false ? ' selected ' : '' ) .'>Bahasa Inggris</option>';
        echo '</select>';
    }
?>
